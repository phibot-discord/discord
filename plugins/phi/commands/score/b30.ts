...
export default defineCommand({
  ...
  async execute(ctx, options) {
    ...
    const data = await b19Card(got.rt, got.save, ctx.db, ctx.userId, ctx.service<Catalog>("phi.catalog"), { nnum, mode: "b30", locale: ctx.locale })
    await replyCard(ctx, "phi/b19/b19", data, "b30")
  },
})
